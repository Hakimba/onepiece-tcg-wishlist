...
export const updateCard = (card: Card, oldId?: string) =>
  Effect.gen(function* () {
    const repo = yield* CardRepository
    const updated = yield* repo.update(card, oldId as any)
...
export const deleteCard = (id: string) =>
  Effect.gen(function* () {
    const repo = yield* CardRepository
    const updated = yield* repo.remove(id as any)
    return AppAction.CardsUpdated({ cards: updated }) as AppActionType
  })
...
export const toggleFavorite = (cards: ReadonlyArray<Card>, id: string) =>
  Effect.gen(function* () {
